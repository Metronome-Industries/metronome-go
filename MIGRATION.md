...
<a href="https://pkg.go.dev/github.com/Metronome-Industries/metronome-go/v3"><img src="https://pkg.go.dev/badge/github.com/Metronome-Industries/metronome-go/v3.svg" alt="Go Reference"></a>

This SDK includes breaking changes to improve the ergonomics of constructing parameters and accessing responses.

To reduce verbosity, the `metronome.F(...)` and `param.Field[T]` have been removed.
All calls to `metronome.F(...)` can be deleted.
...
The `metronome.F(...)` function and `param.Field[T]` type are no longer present in the new SDK.
...
In all cases other than optional primitives, `metronome.F()` can simply be removed.
For optional primitive types, such as `param.Opt[string]`, you can use `metronome.String(string)` to construct the value.
Similar functions exist for other primitive types like `metronome.Int(int)`, `metronome.Bool(bool)`, etc.
...
```diff
foo = FooParams{
-    RequiredString: metronome.String("hello"),
+    RequiredString: "hello",

-    OptionalString: metronome.String("hi"),
+    OptionalString: metronome.String("hi"),

-    Array: metronome.F([]BarParam{
-        BarParam{Prop: ... }
-    }),
+    Array: []BarParam{
+        BarParam{Prop: ... }
...
-    RequiredObject: metronome.F(BarParam{ ... }),
+    RequiredObject: BarParam{ ... },

-    OptionalObject: metronome.F(BarParam{ ... }),
+    OptionalObject: BarParam{ ... },

-    StringEnum: metronome.F[BazEnum]("baz-ok"),
+    StringEnum: "baz-ok",
}
```
...
The `metronome.Raw[T](any)` function has been removed. All request structs now support a
